...
export default function AdminRouteNavigation() {
    const { user } = useUser();
    ...
    const performStatusUpdate = async (delivery: Delivery, nextStatus: Delivery['status'], finalMethod?: 'pix' | 'cash') => {
        ...
        batch.set(clientNotifRef, {
            ...
            description: nextStatus === 'in-progress' 
                ? 'O administrador retirou seu pedido e já está na rota de entrega.' 
    ...
    const simulatePixConfirmation = () => {
        ...
        setTimeout(() => {
            ...
            toast({ title: "Pix Recebido!", description: "Pagamento identificado." });
        }, 3000);
    };
    ...
    if (!deliveries || deliveries.length === 0) {
        return (
            <div className="h-full bg-background flex flex-col items-center justify-center p-6 text-center">
                ...
                <h2 className="text-xl font-bold">Nenhuma entrega para realizar</h2>
                <p className="text-muted-foreground mt-2">Aceite um pedido no painel principal para ver a rota aqui.</p>
                <Button asChild className="mt-6 rounded-2xl px-10 h-14 font-bold shadow-lg"><Link href="/admin">Voltar ao Painel</Link></Button>
            </div>
        );
    }
        ...
        <div className="h-full relative overflow-hidden bg-background">
            <header className="absolute top-0 left-0 right-0 z-20 flex items-center bg-background/90 backdrop-blur-md p-4 justify-between border-b shadow-sm">
                ...
                <div className="text-center">
                    <h2 className="text-sm font-bold uppercase tracking-widest font-headline">Roteiro Live</h2>
                    <p className="text-primary text-[10px] font-bold uppercase">{deliveries.length} Paradas</p>
                </div>
            ...
            <div className={cn(
                ...
                <div className="px-6 h-full overflow-y-auto pb-32">
                    <div className="flex justify-between items-center mb-6">
                        <div>
                            <h3 className="text-2xl font-black font-headline">Sua Rota</h3>
                            <p className="text-sm text-muted-foreground font-medium">{stops.length} destinos restantes</p>
                        </div>
                        <Badge className="bg-primary/10 text-primary border-primary/20">GPS VPS Live</Badge>
                    </div>
                                        ...
                                        <div className="flex justify-between items-start">
                                            <div>
                                                <p className={cn(
                                                    ...
                                                    {isDelayed ? '⚠ ATRASO' : (stop.type === 'pickup' ? 'Coletar em:' : 'Entregar em:')}
                                                </p>
                ...
                <DialogContent className="max-w-[90vw] rounded-3xl p-6">
                    <DialogHeader>
                        <DialogTitle className="font-headline text-2xl font-black text-center text-foreground">
                            {pixStep === 'choice' ? 'Pagamento Direto' : 'Receber via Pix'}
                        </DialogTitle>
                        <DialogDescription className="text-center">
                            {pixStep === 'choice' 
                                ? 'Este cliente deve pagar agora. Como ele fará o pagamento?' 
                                : 'Mostre o código abaixo para o cliente pagar.'}
                        </DialogDescription>
                    </DialogHeader>
                    ...
                    {pixStep === 'qrcode' && (
                        <div className="flex flex-col items-center py-6">
                            ...
                            <div className="mt-6 text-center w-full">
                                <p className="text-xs font-bold text-muted-foreground uppercase tracking-widest mb-1">Valor Final</p>
                    ...
                    {pixStep === 'confirmed' && (
                        <div className="flex flex-col items-center py-10 text-center animate-in zoom-in-95 duration-300">
                            ...
                            <h3 className="text-2xl font-black font-headline text-emerald-600">Recebido com Sucesso!</h3>
                            <Button 
                    ...
                    <Button variant="ghost" className="w-full text-muted-foreground font-bold" onClick={() => setIsPaymentDialogOpen(false)}>Sair</Button>
                </DialogContent>
            </Dialog>
        </div>
    );
