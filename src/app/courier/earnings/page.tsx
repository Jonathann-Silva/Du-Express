...
export default function CourierEarningsPage() {
    ...
    const { dateRangeStart, dateRangeEnd, periodLabel } = useMemo(() => {
        ...
        return { dateRangeStart: weekStart, dateRangeEnd: weekEnd, periodLabel: `${format(weekStart, 'dd/MM')} até ${format(weekEnd, 'dd/MM')}` };
    }, [currentDate]);
    ...
    useEffect(() => {
        ...
        const fetchEarnings = async () => {
            ...
            try {
                ...
                setDeliveries(snapshot.docs.map(doc => ({ ...doc.data(), id: doc.id } as Delivery)));
            } catch (error: any) {
                if (error.code === 'permission-denied') {
                    errorEmitter.emit('permission-error', new FirestorePermissionError({ path: 'deliveries', operation: 'list' }));
                }
                setDeliveries([]);
            } finally { deliveriesLoading && setDeliveriesLoading(false); }
        };
        fetchEarnings();
    ...
    return (
        <div className="flex flex-col h-full bg-background">
            <header className="flex items-center justify-between px-4 pt-6 pb-2 bg-background sticky top-0 z-10 shrink-0">
                <Button asChild variant="ghost" size="icon" className="rounded-full"><Link href="/courier"><ArrowLeft /></Link></Button>
            ...
            <main className="flex-1 overflow-y-auto px-4 pb-24">
                <section className="mt-4">
                    <Card className="p-3 bg-card border shadow-sm rounded-2xl flex items-center justify-between">
                        <Button variant="ghost" size="icon" onClick={handlePrev} className="rounded-full"><ChevronLeft className="size-5" /></Button>
                        ...
                        <Button variant="ghost" size="icon" onClick={handleNext} className="rounded-full"><ChevronRight className="size-5" /></Button>
                    </Card>
                </section>

                <div className={cn("mt-4 p-8 rounded-[2rem] text-white shadow-xl flex flex-col items-center text-center relative overflow-hidden", finalBalance >= 0 ? "bg-primary shadow-primary/20" : "bg-destructive shadow-destructive/20")}>
                    <div className="absolute top-0 right-0 -mr-8 -mt-8 opacity-10"><Banknote size={160} /></div>
                    <p className="text-xs font-bold uppercase tracking-widest opacity-80 mb-1">{finalBalance >= 0 ? "Saldo a Receber" : "Saldo Devedor (Admin)"}</p>
                    {isLoading ? <Skeleton className="h-10 w-48 bg-white/20" /> : <h2 className="text-4xl font-black font-headline">{Math.abs(finalBalance).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</h2>}
                    {finalBalance < 0 && <p className="text-[10px] font-bold bg-black/20 px-3 py-1 rounded-full mt-2">Valor excedente recebido em mãos</p>}
                </div>

                <div className="grid grid-cols-2 gap-3 mt-4">
                    <Card className="p-4 bg-emerald-500/10 border-emerald-500/20">
                        <p className="text-[10px] font-black uppercase text-emerald-600">Ganhos (Sua Parte)</p>
                        <p className="text-lg font-bold text-emerald-700">{stats.totalEarned.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</p>
                    </Card>
                    <Card className="p-4 bg-amber-500/10 border-amber-500/20">
                        <p className="text-[10px] font-black uppercase text-amber-600">Recebido em Mãos</p>
                        <p className="text-lg font-bold text-amber-700">{stats.receivedInCash.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</p>
                    </Card>
                </div>

                <div className="mt-8 space-y-4">
                    <h3 className="text-sm font-bold text-muted-foreground uppercase tracking-widest px-1">Histórico Detalhado</h3>
                    <div className="space-y-3">
                        {isLoading ? Array.from({length: 3}).map((_, i) => <Skeleton key={i} className="h-24 w-full rounded-2xl" />) : deliveries?.map((d) => (
                            <Card key={d.id} className={cn("p-4 rounded-2xl border-l-4 shadow-sm", d.paymentMethod === 'cash' ? "border-l-amber-500 bg-amber-50/30" : "border-l-emerald-500")}>
                                <div className="flex justify-between items-center">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <p className="text-sm font-bold leading-none"><ClientName clientId={d.clientId} /></p>
                                    ...
                                    <div className="text-right">
                                        ...
                                        {d.paymentMethod === 'cash' && <p className="text-[8px] font-bold text-destructive">Coletado: {d.price.toLocaleString('pt-BR', {style:'currency', currency:'BRL'})}</p>}
                                    </div>
                                </div>
                            </Card>
                        ))}
